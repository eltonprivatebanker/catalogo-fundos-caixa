/* =========================================================
   DI FUTURO · TESTE v716
   Integração visual nativa com widgets same-origin.
   ========================================================= */
(function jurosFuturosTesteV716(){
  'use strict';

  const CURRENT_ID = 'diCurrentFrameV716';
  const HISTORY_ID = 'diHistoryFrameV716';
  ...
  function injectNativeSkin(frame, kind){
    ...
    if(doc.getElementById('catalogoNativeSkinV716')) return true;

    const style = doc.createElement('style');
    style.id = 'catalogoNativeSkinV716';
    style.textContent = `
      html,body{
        margin:0!important;
        padding:0!important;
  ...
  function prepare(frame, kind){
    if(!frame || frame.dataset.nativeV716 === '1') return;
    frame.dataset.nativeV716 = '1';
    ...
    const run = () => {
      ...
      try{
        ...
        if(widget && window.ResizeObserver && !frame.__nativeRoV716){
          const ro = new ResizeObserver(() => resize(frame));
          ro.observe(widget);
          frame.__nativeRoV716 = ro;
        }
      }catch(e){}
    };
  ...
  function bind(){
    ...
    const details = document.getElementById('diHistoryDetailsV716');
    if(details && details.dataset.boundV716 !== '1'){
      details.dataset.boundV716 = '1';
  ...
  window.addEventListener('resize', () => {
    ...
    const d = document.getElementById('diHistoryDetailsV716');
    if(d?.open) resize(document.getElementById(HISTORY_ID));
  }, {passive:true});

  window.__ELTAUM_DI_FUTURO_TESTE_V716__ = {
    build:'ELTAUM_DI_FUTURO_TESTE_V716',
    refresh(){
      const current = document.getElementById(CURRENT_ID);
  ...
  console.info('[Catálogo CAIXA] DI Futuro teste v716 nativo ativo');
})();
