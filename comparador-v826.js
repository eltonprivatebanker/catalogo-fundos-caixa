/* ============================================================
   ...
   ============================================================ */
(function comparadorV826(){
  'use strict';

  var TROPHY_V826_DATA='data:image/png;base64,';
  ...
  function enforceViewportV823(){
    ...
    os.setProperty('top','0px','important');
    os.setProperty('right','auto','important');
    os.setProperty('bottom','auto','important');
    os.setProperty('left','0px','important');
    os.setProperty('width',vw+'px','important');
    os.setProperty('min-width',vw+'px','important');
    os.setProperty('max-width',vw+'px','important');
    os.setProperty('height',vh+'px','important');
    os.setProperty('min-height',vh+'px','important');
    os.setProperty('max-height',vh+'px','important');
    os.setProperty('margin','0','important');
    os.setProperty('padding',padY+'px '+padX+'px','important');
    os.setProperty('box-sizing','border-box','important');
    os.setProperty('display','flex','important');
    os.setProperty('align-items','flex-start','important');
    os.setProperty('justify-content','center','important');
    os.setProperty('transform','none','important');
    os.setProperty('translate','none','important');

    var panel=overlay.querySelector('.compar-panel');
    if(panel){
      var ps=panel.style;
      ps.setProperty('position','relative','important');
      ps.setProperty('top','auto','important');
      ps.setProperty('right','auto','important');
      ps.setProperty('bottom','auto','important');
      ps.setProperty('left','auto','important');
      ps.setProperty('width',(vw-(padX*2))+'px','important');
      ps.setProperty('min-width','0','important');
      ps.setProperty('max-width',(vw-(padX*2))+'px','important');
      ps.setProperty('height','auto','important');
      ps.setProperty('min-height','0','important');
      ps.setProperty('max-height',(vh-(padY*2))+'px','important');
      ps.setProperty('margin','0','important');
      ps.setProperty('transform','none','important');
      ps.setProperty('translate','none','important');
      ps.setProperty('box-sizing','border-box','important');
    }

    /*
      Se algum ancestral/engine ainda criar containing block deslocado,
      mede o retângulo real e compensa exatamente em pixels de viewport.
    */
    requestAnimationFrame(function(){
      if(!overlay || !isOpen()) return;
      os.setProperty('transform','none','important');
      var r=overlay.getBoundingClientRect();
      var dx=Math.abs(r.left)>0.5 ? -r.left : 0;
      var dy=Math.abs(r.top)>0.5 ? -r.top : 0;
      if(dx || dy){
        os.setProperty('transform','translate3d('+dx+'px,'+dy+'px,0)','important');
      }
    });
  }

  function bindViewportResizeV823(){
    if(viewportResizeBoundV823) return;
  ...
  function resetHorizontalScrollV822(force){
    if(!overlay) return;
    var scroller=overlay.querySelector('.compar-scroll');
    if(!scroller) return;
    var run=function(){
      if(force || scroller.scrollLeft!==0) scroller.scrollLeft=0;
    };
    run();
  ...
  function stripLegacyWinnerDecorations(){
    ...
    Array.from(table.querySelectorAll('td,th')).forEach(function(cell){
      cell.classList.remove('compar-winner-soft-v820','compar-winner-valid-v821','compar-winner-trophy-v824','compar-winner-trophy-v825');
      cell.removeAttribute('data-trophy-v825');
      cell.removeAttribute('data-winner-v826');
      cell.removeAttribute('data-winner-tie-v826');
      if(cell.title==='Melhor valor entre os fundos selecionados') cell.removeAttribute('title');

      Array.from(cell.querySelectorAll('.compar-trophy-v824,.compar-trophy-v825')).forEach(function(el){el.remove();});
  ...
  function comparableWinnerMap(model){
    ...
    specs.forEach(function(spec){
      ...
      if(!row) return;
      var bestNumber=null,values=[];
      row.values.forEach(function(value,index){
        var n=parseMetricNumber(value);
        values.push({index:index,number:n});
        if(n==null) return;
        if(bestNumber==null || (spec[1]==='min'?n<bestNumber:n>bestNumber)) bestNumber=n;
      });
      if(bestNumber==null) return;
      var eps=Math.max(1e-9,Math.abs(bestNumber)*1e-10);
      var indices=values.filter(function(x){
        return x.number!=null && Math.abs(x.number-bestNumber)<=eps;
      }).map(function(x){return x.index;});
      if(indices.length){
        map[String(row.label||'').toLowerCase()]={
          fundIndex:indices[0],
          fundIndices:indices,
          isTie:indices.length>1,
          rowLabel:row.label,
          mode:spec[1],
  ...
  function applyComparableWinners(model){
    ...
    Array.from(table.querySelectorAll('tr')).forEach(function(tr){
      ...
      if(!winner) return;
      (winner.fundIndices||[winner.fundIndex]).forEach(function(idx){
        var cell=cells[idx+1];
        if(!cell) return;
        cell.setAttribute('data-winner-v826','1');
        if(winner.isTie) cell.setAttribute('data-winner-tie-v826','1');
        cell.title=winner.isTie
          ? 'Melhor valor nesta métrica · empate entre os fundos selecionados'
          : 'Melhor valor nesta métrica entre os fundos selecionados';
      });
    });
  }
  ...
  function buildPrintHTML(model){
    var printWinners=comparableWinnerMap(model);
    var sections='';
    var current='';
    model.rows.forEach(function(row){
      ...
      var win=printWinners[String(row.label||'').toLowerCase()];
      sections+='<tr><th>'+esc(row.label)+'</th>'+row.values.map(function(v,i){
        var trophy=win && (win.fundIndices||[win.fundIndex]).indexOf(i)>=0 ? '<span class="trophy">🏆</span>' : '';
        return '<td class="'+printableValueClass(v)+(trophy?' winner':'')+'">'+trophy+esc(v)+'</td>';
      }).join('')+'</tr>';
    });
  ...
  function directPDF(model,JsPDF){
    var pdfWinners=comparableWinnerMap(model);
    ...
    doc.autoTable({
      ...
      didParseCell:function(data){
        ...
        if(data.column.index>0){
          var rowLabel=String(data.row.raw && data.row.raw[0] || '').trim().toLowerCase();
          var win=pdfWinners[rowLabel];
          if(win && (win.fundIndices||[win.fundIndex]).indexOf(data.column.index-1)>=0){
            data.cell.styles.fillColor=[252,249,242];
            data.cell.styles.fontStyle='bold';
            data.cell.styles.cellPadding=1.65;
            data.cell.text=(data.cell.text||[]).map(function(t){return '      '+String(t);});
          }
        }
      },
      didDrawCell:function(data){
        if(data.section!=='body' || data.column.index<=0 || sectionRows[data.row.index]) return;
        var rowLabel=String(data.row.raw && data.row.raw[0] || '').trim().toLowerCase();
        var win=pdfWinners[rowLabel];
        if(!win || (win.fundIndices||[win.fundIndex]).indexOf(data.column.index-1)<0) return;
        var size=3.6;
        var x=data.cell.x+1.45;
        var y=data.cell.y+(data.cell.height-size)/2;
        try{doc.addImage(TROPHY_V826_DATA,'PNG',x,y,size,size,undefined,'FAST');}catch(e){}
      },
  ...
  function buildExcelProfessional(ExcelJS,model){
    ...
    var trophyImageV826=null;
    try{trophyImageV826=wb.addImage({base64:TROPHY_V826_DATA,extension:'png'});}catch(e){}
    ...
    model.rows.forEach(function(row){
      ...
      if(win && win.fundIndex>=0){
        (win.fundIndices||[win.fundIndex]).forEach(function(idx){
          var wc=er.getCell(idx+2);
          ...
          wc.note=win.isTie?'Melhor valor nesta métrica · empate entre os fundos selecionados':'Melhor valor nesta métrica entre os fundos selecionados';
          // Mantém o valor original e seu formato numérico. O troféu é uma imagem ancorada.
          wc.alignment=Object.assign({},wc.alignment||{},{vertical:'middle',wrapText:true,indent:2});
          if(trophyImageV826!=null){
            try{
              ws.addImage(trophyImageV826,{
                tl:{col:idx+1.08,row:er.number-1+0.17},
  ...
  function fallbackSimpleExcel(model){
    return loadSheetJS().then(function(XLSX){
      ...
      model.rows.forEach(function(r){
        ...
        var win=winners[String(r.label||'').toLowerCase()];
        var vals=r.values.map(function(v,i){
          return win && (win.fundIndices||[win.fundIndex]).indexOf(i)>=0 ? '🏆 '+normalizeMetricDisplay(v) : v;
        });
        rows.push([r.label].concat(vals));
      });
  ...
  function bindPortalObserver(){
    ...
    portalObserver=new MutationObserver(function(){
      if(overlay && overlay.parentElement!==document.body) portalOverlay();
      if(isOpen()) enforceViewportV823();
    });
  ...
  function init(){
    ...
    if(overlay.dataset.v826Bound==='1') return;
    overlay.dataset.v826Bound='1';
    removeOldExportUI(); injectExportButtons(); wrapLegacyOpen(); bindPortalObserver(); bindViewportResizeV823(); syncFundCount(); syncOpenState(); bindObservers(); queueRefresh();
    if(isOpen()) enforceViewportV823();
