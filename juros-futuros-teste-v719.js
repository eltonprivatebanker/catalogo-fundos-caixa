/* =========================================================
   DI FUTURO · TESTE v719
   Bloco limpo + integração visual nativa.
   ========================================================= */
(function jurosFuturosTesteV719(){
  'use strict';

  const CURRENT_ID = 'diCurrentFrameV719';
  const HISTORY_ID = 'diHistoryFrameV719';
  ...
  function injectNativeSkin(frame, kind){
    ...
    if(doc.getElementById('catalogoNativeSkinV719')) return true;

    const style = doc.createElement('style');
    style.id = 'catalogoNativeSkinV719';
    style.textContent = `
      html,body{
        margin:0!important;
        padding:0!important;
  ...
  function prepare(frame, kind){
    if(!frame || frame.dataset.nativeV719 === '1') return;
    frame.dataset.nativeV719 = '1';
    ...
    const run = () => {
      ...
      try{
        ...
        if(widget && window.ResizeObserver && !frame.__nativeRoV719){
          const ro = new ResizeObserver(() => resize(frame));
          ro.observe(widget);
          frame.__nativeRoV719 = ro;
        }
      }catch(e){}
    };
  ...
  function bind(){
    ...
    const details = document.getElementById('diHistoryDetailsV719');
    if(details && details.dataset.boundV719 !== '1'){
      details.dataset.boundV719 = '1';
  ...
  window.addEventListener('resize', () => {
    ...
    const d = document.getElementById('diHistoryDetailsV719');
    if(d?.open) resize(document.getElementById(HISTORY_ID));
  }, {passive:true});

  window.__ELTAUM_DI_FUTURO_TESTE_V719__ = {
    build:'ELTAUM_DI_FUTURO_TESTE_V719',
    refresh(){
      const current = document.getElementById(CURRENT_ID);
  ...
  console.info('[Catálogo CAIXA] DI Futuro teste v719 nativo ativo');
})();
