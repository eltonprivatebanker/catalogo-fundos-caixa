/* ============================================================
   V824 — COMPARADOR · TROFÉUS SELETIVOS
...
(function comparadorV824(){
  ...
  function stripLegacyWinnerDecorations(){
    ...
    Array.from(table.querySelectorAll('td,th')).forEach(function(cell){
      cell.classList.remove('compar-winner-soft-v820','compar-winner-valid-v821','compar-winner-trophy-v824');
      if(cell.title==='Melhor valor entre os fundos selecionados') cell.removeAttribute('title');

      Array.from(cell.querySelectorAll('.compar-trophy-v824')).forEach(function(el){el.remove();});
  ...
  function applyComparableWinners(model){
    ...
    Array.from(table.querySelectorAll('tr')).forEach(function(tr){
      ...
      (winner.fundIndices||[winner.fundIndex]).forEach(function(idx){
        ...
        if(!cell) return;
        cell.classList.add('compar-winner-valid-v821','compar-winner-trophy-v824');
        var title=winner.isTie?'Melhor valor nesta métrica · empate entre os fundos selecionados':'Melhor valor nesta métrica entre os fundos selecionados';
        cell.title=title;
        if(!cell.querySelector('.compar-trophy-v824')){
          var trophy=document.createElement('span');
          trophy.className='compar-trophy-v824';
          trophy.setAttribute('aria-hidden','true');
          trophy.textContent='🏆';
          trophy.title=title;
          cell.insertBefore(trophy,cell.firstChild);
        }
      });
    });
  }
  ...
  function directPDF(model,JsPDF){
    ...
    doc.autoTable({
      ...
      didParseCell:function(data){
        ...
        if(data.column.index>0){
          ...
          if(win && (win.fundIndices||[win.fundIndex]).indexOf(data.column.index-1)>=0){
            ...
            data.cell.styles.cellPadding={top:1.65,right:1.65,bottom:1.65,left:4.6};
          }
        }
      },
      didDrawCell:function(data){
        ...
        var x=data.cell.x+1.35, y=data.cell.y+data.cell.height/2-1.25;
        doc.setDrawColor(201,151,58);doc.setFillColor(201,151,58);doc.setLineWidth(.28);
        // Taça vetorial pequena: copa + alças + haste + base.
        doc.roundedRect(x,y,2.7,1.65,.35,.35,'S');
        doc.line(x+.35,y+1.65,x+1.05,y+2.35);
        doc.line(x+2.35,y+1.65,x+1.65,y+2.35);
        doc.line(x+1.35,y+2.15,x+1.35,y+3.0);
        doc.line(x+.65,y+3.0,x+2.05,y+3.0);
        doc.line(x,y+.35,x-.55,y+.35);doc.line(x-.55,y+.35,x-.55,y+1.05);doc.line(x-.55,y+1.05,x+.15,y+1.25);
        doc.line(x+2.7,y+.35,x+3.25,y+.35);doc.line(x+3.25,y+.35,x+3.25,y+1.05);doc.line(x+3.25,y+1.05,x+2.55,y+1.25);
      },
    ...
    model.rows.forEach(function(row){
      ...
      if(win && win.fundIndex>=0){
        (win.fundIndices||[win.fundIndex]).forEach(function(idx){
          ...
          if(typeof wc.value==='number'){
            var baseFmt=wc.numFmt||'0.00';
            wc.numFmt='"🏆 " '+baseFmt;
          }else{
            wc.value='🏆 '+String(wc.value==null?'':wc.value);
          }
        });
      }
    });
  ...
  function init(){
    ...
    if(overlay.dataset.v824Bound==='1') return;
    overlay.dataset.v824Bound='1';
