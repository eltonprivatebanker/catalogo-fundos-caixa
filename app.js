...
(function(){
  ...
  function applyFilterPreset(preset){
    ...
    // IMPORTANTE: não zerar os demais filtros aqui.
    // Este bloco trabalha em funil: Perfil + Categoria + Benchmark + Risco.
  ...
  function updatePresetStates(){
    ...
    qsa('.filter-preset-chip').forEach(btn => {
      ...
      if(p === 'all'){
        on = !cat && !bench && !risco && !perfil && !(typeof hideSemDados !== 'undefined' && hideSemDados);
      }
      ...
      // Categoria
      if(p === 'renda-fixa-simples') on = cat === 'RENDA FIXA SIMPLES';
      if(p === 'renda-fixa') on = cat === 'RENDA FIXA';
      if(p === 'renda-fixa-referenciado') on = cat === 'RENDA FIXA REFERENCIADO';
      if(p === 'renda-fixa-curto-prazo') on = cat === 'RENDA FIXA CURTO PRAZO';
      if(p === 'multimercado') on = cat === 'MULTIMERCADO';
      if(p === 'cambial') on = cat === 'CAMBIAL';
      if(p === 'acoes') on = cat === 'ACOES';
      if(p === 'fundo-de-indice') on = cat === 'FUNDO DE INDICE';
...
(function(){
  ...
  const GROUP_PRESETS=new Set([]); // v funil: categorias também usam activeCat, não filtro direto
  const VALID_PRESETS=new Set(['all','favoritos','pf','cdi','conservador','ipca','renda-fixa-simples','renda-fixa','renda-fixa-referenciado','renda-fixa-curto-prazo','multimercado','acoes','cambial','fundo-de-indice','fmp']);
  ...
  function syncShortcutVisual(preset){
    // v funil: permite vários atalhos ativos ao mesmo tempo.
    const cat = typeof activeCat !== 'undefined' ? activeCat : '';
    const bench = typeof activeBenchmark !== 'undefined' ? activeBenchmark : '';
    const risco = typeof activeRisco !== 'undefined' ? activeRisco : '';
    const perfil = typeof activePerfil !== 'undefined' ? activePerfil : '';
    const semDados = typeof hideSemDados !== 'undefined' ? !!hideSemDados : false;
    const fav = !!window.__favListMode;
    const hasAny = !!(cat || bench || risco || perfil || semDados || fav);
    ...
    const parts = [];
    if(perfil) parts.push(perfil);
    if(cat) parts.push(cat);
    if(bench) parts.push(bench);
    if(risco) parts.push(risco);
    if(semDados) parts.push('sem pipeline');
    if(fav) parts.push('favoritos');

    const summary=document.getElementById('mobileFilterSummary');
    if(summary) summary.textContent = 'Filtros: ' + (parts.length ? parts.join(' · ') : 'todos');
    ...
    try{ if(typeof updateMobileFilterSummary === 'function') updateMobileFilterSummary(); }catch(e){}
  }

  function resetBaseFiltersForShortcut(){
    // v funil: não limpa activeCat/activeBenchmark/activePerfil/activeRisco.
    // A limpeza total fica exclusivamente no atalho "Todos".
  ...
  function applyShortcutPreset(preset){
    ...
    keepFundAreaStable(()=>{
      if(preset==='favoritos'){
        ...
        try{ if(typeof toggleFavList==='function') toggleFavList(); else if(typeof applyFilter==='function') applyFilter(); }catch(e){ console.warn('[Atalhos v funil] favoritos falhou:',e); }
        try{ if(typeof syncFilterControls==='function') syncFilterControls(); }catch(e){}
      ...
      if(preset==='all'){
        ...
        try{ if(typeof syncFilterControls==='function') syncFilterControls(); }catch(e){}
        try{ if(typeof applyFilter==='function') applyFilter(); }catch(e){ console.warn('[Atalhos v funil] todos falhou:',e); }
      ...
      try{
        ...
        // Benchmark
        else if(preset==='cdi') activeBenchmark='CDI';
        ...
        // Categoria
        else if(preset==='renda-fixa-simples') activeCat='RENDA FIXA SIMPLES';
        else if(preset==='renda-fixa') activeCat='RENDA FIXA';
        else if(preset==='renda-fixa-referenciado') activeCat='RENDA FIXA REFERENCIADO';
        else if(preset==='renda-fixa-curto-prazo') activeCat='RENDA FIXA CURTO PRAZO';
        else if(preset==='multimercado') activeCat='MULTIMERCADO';
        else if(preset==='cambial') activeCat='CAMBIAL';
        else if(preset==='acoes') activeCat='ACOES';
        else if(preset==='fundo-de-indice') activeCat='FUNDO DE INDICE';
        else if(preset==='fmp') activeCat='FUNDOS MUTUOS DE PRIVATIZACAO';
      }catch(e){}

      try{ window.__favListMode=false; }catch(e){}
      try{ if(typeof syncFilterControls==='function') syncFilterControls(); }catch(e){}
      try{ if(typeof applyFilter==='function') applyFilter(); }catch(e){ console.warn('[Atalhos v funil] aplicar filtro falhou:',e); }
      syncShortcutVisual(preset);
    });
  }
...
/* ════════════════════════════════════════════════════════════════════
   PATCH FINAL — Funil PF + categoria + benchmark + risco
   Marcador: ELTAUM_FUNIL_PF_20260604_01
════════════════════════════════════════════════════════════════════ */
window.__diagnosticarFunilPF = function(){
  return {
    build:'ELTAUM_FUNIL_PF_20260604_01',
    activePerfil: typeof activePerfil !== 'undefined' ? activePerfil : null,
    activeCat: typeof activeCat !== 'undefined' ? activeCat : null,
    activeBenchmark: typeof activeBenchmark !== 'undefined' ? activeBenchmark : null,
    activeRisco: typeof activeRisco !== 'undefined' ? activeRisco : null,
    hideSemDados: typeof hideSemDados !== 'undefined' ? hideSemDados : null,
    filtrosAtivos: document.getElementById('activeFilterStrip')?.textContent?.trim() || '',
    resultado: document.getElementById('resultInfo')?.textContent || '',
    atalhos:[...document.querySelectorAll('.shortcut-preset[data-preset], .filter-preset-chip[data-preset]')].map(b=>({
      texto:b.textContent.trim(), preset:b.dataset.preset, ativo:b.classList.contains('active'), aria:b.getAttribute('aria-pressed')
    }))
  };
};
console.info('[Catálogo CAIXA] Funil PF ativo: ELTAUM_FUNIL_PF_20260604_01');
