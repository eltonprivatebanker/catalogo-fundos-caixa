...
async function carregarDados() {
    try {
        ...
        // Verifica onde está a verdadeira lista de fundos dentro do JSON
        let listaDeFundos = [];
        
        if (Array.isArray(dadosBrutos)) {
            listaDeFundos = dadosBrutos; // Se já for uma lista direta
        } else if (dadosBrutos.value && Array.isArray(dadosBrutos.value)) {
            listaDeFundos = dadosBrutos.value; // Se estiver no formato OData
        } else if (dadosBrutos.data && Array.isArray(dadosBrutos.data)) {
            listaDeFundos = dadosBrutos.data; // Se estiver embrulhado em "data"
        } else {
            console.error("Estrutura do JSON não reconhecida:", dadosBrutos);
            throw new Error("A lista de fundos não foi encontrada no JSON.");
        }
        ...
        document.querySelector('#tabela-fundos tbody').innerHTML = `
            <tr><td colspan="3" class="text-red align-center" style="text-align: center; padding: 20px;">
                ⚠️ Falha ao carregar os dados. Verifique o console para mais detalhes.
            </td></tr>
        `;
    }
}
