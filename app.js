...
/* ════════════════════════════════════════════════════════════
   v42 — filtro mobile mínimo, estável e com select nativo
   Remove a dependência visual dos chips avançados no celular.
════════════════════════════════════════════════════════════ */
(function(){
  'use strict';
  const BUILD_V42='ELTAUM_FILTRO_MOBILE_MINIMO_20260605_v42';
  ...
  function setBtnText(){
    const t=qs('#filterButtonText');
    if(t) t.textContent='Filtrar fundos';
  ...
  function applyClassFromSelect(value){
    ...
    const select=qs('#filterClassSelectV42') || qs('#filterClassSelectV37');
    if(select && select.value!==cat) select.value=cat;

    // Sincroniza com o motor legado de filtros, quando os chips existirem.
    const row=qs('#catFilters');
    if(row){
      row.querySelectorAll('[data-cat]').forEach(b=>b.classList.toggle('active',(b.dataset.cat||'')===cat));
    }
    qsa('#benchmarkFilters [data-benchmark], #perfilFilters [data-perfil], #riscoFilters [data-risco]').forEach(b=>{
      const isAll=(b.dataset.benchmark==='' || b.dataset.perfil==='' || b.dataset.risco==='');
      b.classList.toggle('active',!!isAll);
    });

    try{ if(typeof applyFilter==='function') applyFilter(); }catch(e){ console.warn('[v42] Falha ao aplicar filtro:',e); }
    try{ if(typeof updateMobileFilterSummary==='function') updateMobileFilterSummary(); }catch(e){}
    setBtnText();
  }

  function clearV42Filters(){
    try{
      activeCat='';
      activeBenchmark='';
      activePerfil='';
  ...
  function closeFilters(){
    ...
    const backdrop=qs('#filterBackdrop');
    if(sec){
      sec.classList.add('mobile-filters-collapsed','desktop-filters-collapsed');
      sec.classList.remove('open','is-open');
    }
  ...
  function openFilters(){
    ...
    const backdrop=qs('#filterBackdrop');
    if(sec){
      sec.classList.remove('mobile-filters-collapsed','desktop-filters-collapsed');
      sec.classList.add('open','is-open','filters-minimal-v42');
    }
  ...
  function bindV42(){
    document.documentElement.dataset.filterBuild=BUILD_V42;
    const meta=qs('meta[name="app-build"]');
    if(meta) meta.setAttribute('content',BUILD_V42);

    const drawer=qs('#fundFilterDrawer');
    if(drawer){
      drawer.classList.add('filters-minimal-v42');
      drawer.classList.remove('filters-select-v37');
    }

    // Garante que a UI antiga não volte a aparecer em mobile por renderizações posteriores.
    qsa('#fundFilterDrawer .filter-refine-block-v37, #fundFilterDrawer .filter-advanced-line-v37').forEach(el=>{
    ...
    const select=qs('#filterClassSelectV42') || qs('#filterClassSelectV37');
    if(select && !select.dataset.v42Bound){
      select.dataset.v42Bound='1';
      try{ select.value=activeCat||''; }catch(e){}
      select.addEventListener('change',()=>applyClassFromSelect(select.value));
    }

    const toggle=qs('#mobileFilterToggle');
    if(toggle && !toggle.dataset.v42Bound){
      toggle.dataset.v42Bound='1';
      toggle.addEventListener('click',ev=>{
        if(!isMobile()) return;
        ev.preventDefault();
        ev.stopImmediatePropagation();
        const sec=qs('#fundFilterDrawer');
        const open=sec && !sec.classList.contains('mobile-filters-collapsed');
        open ? closeFilters() : openFilters();
      },true);
    }

    const clear=qs('#clearFiltersBtn');
    if(clear && !clear.dataset.v42Bound){
      clear.dataset.v42Bound='1';
    ...
    if(clearTop && !clearTop.dataset.v42Bound){
      clearTop.dataset.v42Bound='1';
      clearTop.addEventListener('click',()=>setTimeout(clearV42Filters,0));
    }

    const apply=qs('#filterApplyBtn');
    if(apply && !apply.dataset.v42Bound){
      apply.dataset.v42Bound='1';
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    bindV42();
    setTimeout(bindV42,250);
    setTimeout(bindV42,900);
  });
  window.addEventListener('resize',()=>setTimeout(bindV42,80));
  window.__eltonFiltroMobileV42={applyClassFromSelect, clearV42Filters, openFilters, closeFilters};
})();
