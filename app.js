Com certeza! Preparei o código completo do `app.js`, já com todas as correções aplicadas.

Fiz duas limpezas fundamentais aqui:

1. **Corrigi a sintaxe quebrada** e removi as duplicatas da função `applyFilterPreset` que ficaram soltas no meio do arquivo.
2. **Ajustei o Patch SIPII no final do arquivo**: removi o `clearCurrentFilters()` de dentro dele (exceto para o botão "Todos"). Isso garante que o seu funil cumulativo vai funcionar perfeitamente em todas as categorias.

Basta copiar o bloco inteiro abaixo e salvar no seu `app.js`:
