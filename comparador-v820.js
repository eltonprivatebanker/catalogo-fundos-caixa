/* ============================================================
   V820 — COMPARADOR · MODO FOCO REAL + PDF/EXCEL PROFISSIONAIS
...
(function comparadorV820(){
    ...
    Array.from(table.querySelectorAll('td,th')).forEach(function(cell){
      ...
      nodes.forEach(function(node){
        if(/[🏆🏅🥇]/.test(node.nodeValue||'')){
          node.nodeValue=(node.nodeValue||'').replace(/[🏆🏅🥇]/g,'').replace(/^\s+/,'');found=true;
        }
      });
      if(found){
        cell.classList.add('compar-winner-soft-v820');
        if(!cell.title) cell.title='Melhor valor entre os fundos selecionados';
      }
    });
  }
  ...
  function directPDF(model,JsPDF){
    ...
    doc.setDrawColor(196,203,214);doc.line(margin,21,W-margin,21);
    ...
    doc.autoTable({
      ...
      didDrawPage:function(data){
        ...
        doc.text('Página '+doc.internal.getNumberOfPages(),W-margin,H-6,{align:'right'});
      }
    });
  ...
  function buildExcelProfessional(ExcelJS,model){
    ...
    sum.mergeCells('A8:F8');sum.getCell('A8').value='Dados para fins informativos e comparativos. Consulte os documentos oficiais do fundo antes da contratação.';sum.getCell('A8').font={italic:true,color:{argb:'FF'+gray},size:9};sum.getCell('A8').alignment={wrapText:true};
    ...
    model.rows.forEach(function(row){
      ...
      model.summary.forEach(function(win){
        if(win.fundIndex>=0 && String(win.rowLabel).toLowerCase()===String(row.label).toLowerCase()){
          var wc=er.getCell(win.fundIndex+2);wc.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FF'+goldLight}};wc.border={left:{style:'medium',color:{argb:'FF'+gold}},bottom:{style:'thin',color:{argb:'FF'+grid}}};wc.font=Object.assign({},wc.font||{},{bold:true});
        }
      });
    });
