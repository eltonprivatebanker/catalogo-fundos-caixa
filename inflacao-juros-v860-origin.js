/* ============================================================
   V860 — Inflação e juros · semântica na origem (desktop)
   ------------------------------------------------------------
   Objetivos:
...
(() => {
  ...
  function applyMetaLegend(card) {
    ...
    items.slice(0, 3).forEach((item, index) => {
      ...
      if (desktop && desktop.textContent !== labels[index]) desktop.textContent = labels[index];
      if (fallback && fallback.textContent !== labels[index]) fallback.textContent = labels[index];
    });
  }
  ...
  function applyDesktop() {
    ...
    moveIpca15ToFlow();
    applySelicFinalText();
    document.documentElement.classList.add('v860-inflacao-ready');
  }
  ...
  function refresh() {
    if (!mq.matches) {
      ...
      headerObserver?.disconnect();
      restoreIpca15();
      document.documentElement.classList.remove('v860-inflacao-ready');
      return;
    }
