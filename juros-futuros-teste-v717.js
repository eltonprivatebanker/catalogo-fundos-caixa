/* =========================================================
   DI FUTURO · TESTE v717
   Acabamento final + integração visual nativa.
   ========================================================= */
(function jurosFuturosTesteV717(){
  'use strict';

  const CURRENT_ID = 'diCurrentFrameV717';
  const HISTORY_ID = 'diHistoryFrameV717';
  ...
  function injectNativeSkin(frame, kind){
    ...
    if(doc.getElementById('catalogoNativeSkinV717')) return true;

    const style = doc.createElement('style');
    style.id = 'catalogoNativeSkinV717';
    style.textContent = `
      html,body{
        margin:0!important;
        padding:0!important;
  ...
  function prepare(frame, kind){
    if(!frame || frame.dataset.nativeV717 === '1') return;
    frame.dataset.nativeV717 = '1';
    ...
    const run = () => {
      ...
      try{
        ...
        if(widget && window.ResizeObserver && !frame.__nativeRoV717){
          const ro = new ResizeObserver(() => resize(frame));
          ro.observe(widget);
          frame.__nativeRoV717 = ro;
        }
      }catch(e){}
    };
  ...
  function bind(){
    ...
    const details = document.getElementById('diHistoryDetailsV717');
    if(details && details.dataset.boundV717 !== '1'){
      details.dataset.boundV717 = '1';
  ...
  window.addEventListener('resize', () => {
    ...
    const d = document.getElementById('diHistoryDetailsV717');
    if(d?.open) resize(document.getElementById(HISTORY_ID));
  }, {passive:true});

  window.__ELTAUM_DI_FUTURO_TESTE_V717__ = {
    build:'ELTAUM_DI_FUTURO_TESTE_V717',
    refresh(){
      const current = document.getElementById(CURRENT_ID);
  ...
  console.info('[Catálogo CAIXA] DI Futuro teste v717 nativo ativo');
})();
